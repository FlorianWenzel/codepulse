...
- **Scanner** (`codepulse-scan`): **15 languages** via tree-sitter (Go, Python, JS, TS, Java, Ruby, Rust, C, Bash, C++, C#, PHP, Kotlin, Scala, Swift); **145 rules** ([full catalogue](docs/RULES_CATALOG.md)) incl. cross-language **taint/dataflow** (command & SQL injection); rules for bugs/vulnerabilities/code-smells/security-hotspots; metrics (LOC, cyclomatic & **cognitive complexity** — both with per-function rules, comments), **duplication detection**, **coverage import** (LCOV/Go/Cobertura), A–E **ratings** + technical debt, **new-code period** via git blame; SARIF 2.1.0 + JSON output; **external-analyzer SARIF import** (consolidate gosec/ESLint/Bandit/…); **quality profiles** (`.codepulse.yml`: disable rules, override severities, tune complexity thresholds, exclude paths); **inline suppression** (`codepulse:ignore` / `NOSONAR`).
