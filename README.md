...
**Status:** Phases 1–4 complete and Phase 5 (SonarQube-parity depth) substantially delivered — all end-to-end tested. Working today:

- **Scanner** (`codepulse-scan`): **15 languages** via tree-sitter (Go, Python, JS, TS, Java, Ruby, Rust, C, Bash, C++, C#, PHP, Kotlin, Scala, Swift); **157 rules** ([full catalogue](docs/RULES_CATALOG.md)) incl. cross-language **taint/dataflow** (SQL/command injection, XSS, SSRF across Go/Python/Java/JS/TS); rules for bugs/vulnerabilities/code-smells/security-hotspots; metrics (LOC, cyclomatic & **cognitive complexity** — both with per-function rules, comments), **duplication detection**, **coverage import** (LCOV/Go/Cobertura), A–E **ratings** + technical debt, **new-code period** via git blame; SARIF 2.1.0 + JSON output; **external-analyzer SARIF import** (consolidate gosec/ESLint/Bandit/…); **quality profiles** (`.codepulse.yml`: disable rules, override severities, tune complexity thresholds, exclude paths); **inline suppression** (`codepulse:ignore` / `NOSONAR`); **secret scanning** (AWS/GitHub/Stripe/Slack tokens, private keys — incl. config files like `.env`/`.yml`/`.pem`); **Dockerfile checks** (latest tag, curl|sh, run-as-root, sudo, ADD); **GitHub Actions** script-injection detection (untrusted `${{ github.event.* }}` in workflows).
...
- **Dashboard** (`web/`): Vue 3 SPA — projects, gate/ratings/measures + file drilldown, findings trend, issue & hotspot triage, security report, gate admin, **rule-catalogue browser** (search + language/type filters, CWE/OWASP); i18n/theme/a11y.
...
New contributor? See **[CONTRIBUTING.md](CONTRIBUTING.md)** for build/test commands and how to add a rule or language.
...
| [docs/RULES_CATALOG.md](docs/RULES_CATALOG.md) | Generated reference of all built-in rules (type, severity, CWE/OWASP) — `make rules-catalog` |
