...
func jsLikeRules(spec langspec.Spec) []Rule {
	...
	return []Rule{
		...
		},
		{
			...
			Severity:  domain.SevMajor,
			EffortMin: 15,
			Query:     `(call_expression function: (member_expression property: (property_identifier) @fn (#eq? @fn "createHash")) arguments: (arguments (string (string_fragment) @algo)) (#match? @algo "^(md5|sha1)$")) @flag`,
			Capture:   "flag",
			Message:   "MD5/SHA-1 are weak; use sha256+ (and a KDF like bcrypt/scrypt/argon2 for passwords).",
		},
		jsTaintEvalRule(p),
		jsTaintXSSRule(p),
		jsTaintExecRule(p),
