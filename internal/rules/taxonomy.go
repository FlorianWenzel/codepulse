...
var remediation = map[string]string{
	...
	"java:unsafe-deserialization": "Avoid Java native deserialization of untrusted data; use JSON, or set a strict ObjectInputFilter allow-list.",
	"js:weak-hash":                "Use crypto sha256+; for passwords use bcrypt/scrypt/argon2, not a raw hash.",
	"ts:weak-hash":                "Use crypto sha256+; for passwords use bcrypt/scrypt/argon2, not a raw hash.",
	"java:weak-hash":              "Use MessageDigest.getInstance(\"SHA-256\") or stronger; PBKDF2/bcrypt/argon2 for passwords.",
	...
	"php:unsafe-deserialization":  "Avoid unserialize() on untrusted input; prefer json_decode, or pass allowed_classes => false.",
...
var ruleTaxonomy = map[string]Taxonomy{
	...
	"java:unsafe-deserialization": {Description: "ObjectInputStream.readObject deserializes arbitrary classes; untrusted data can trigger gadget-chain RCE.", CWE: []string{"CWE-502"}, OWASP: []string{"A08:2021-Software and Data Integrity Failures"}, Tags: []string{"security", "deserialization"}},
	"js:weak-hash":                {Description: "MD5 and SHA-1 are cryptographically broken for security use; prefer SHA-256 or stronger.", CWE: []string{"CWE-327", "CWE-328"}, OWASP: []string{"A02:2021-Cryptographic Failures"}, Tags: []string{"security", "cryptography"}},
	"ts:weak-hash":                {Description: "MD5 and SHA-1 are cryptographically broken for security use; prefer SHA-256 or stronger.", CWE: []string{"CWE-327", "CWE-328"}, OWASP: []string{"A02:2021-Cryptographic Failures"}, Tags: []string{"security", "cryptography"}},
	"java:weak-hash":              {Description: "MD5 and SHA-1 are cryptographically broken for security use; prefer SHA-256 or stronger.", CWE: []string{"CWE-327", "CWE-328"}, OWASP: []string{"A02:2021-Cryptographic Failures"}, Tags: []string{"security", "cryptography"}},
	...
	"php:unsafe-deserialization":  {Description: "unserialize() on untrusted input enables PHP object injection / RCE via magic methods.", CWE: []string{"CWE-502"}, OWASP: []string{"A08:2021-Software and Data Integrity Failures"}, Tags: []string{"security", "deserialization"}},
...
func Catalog() []Meta {
	...
	out = append(out, WorkflowCatalog()...)
	return out
}
