...
func javaRules() []Rule {
	return []Rule{
		...
		},
		{
			...
			Severity:  domain.SevCritical,
			EffortMin: 30,
			Query:     `(method_invocation name: (identifier) @m (#eq? @m "readObject")) @flag`,
			Capture:   "flag",
			Message:   "ObjectInputStream.readObject deserializes arbitrary classes; with attacker-controlled data this enables remote code execution. Use a safe format (JSON) or a strict allow-list (ObjectInputFilter).",
		},
		javaTaintSQLRule(),
		javaTaintExecRule(),
