...
vi.mock('../api.js', () => ({
  default: {
    listRules: vi.fn().mockResolvedValue([
      ...
      { id: 'py:exec-eval', name: 'eval', language: 'python', type: 'VULNERABILITY', severity: 'CRITICAL', cwe: ['CWE-95'], owasp: ['A03:2021-Injection'], remediation: 'Use ast.literal_eval' },
...
describe('Rules catalogue view', () => {
  it('renders all rules with security mappings', async () => {
    ...
    // remediation ("Fix:") surfaced for rules that carry it
    expect(w.findAll('[data-test=rule-fix]')).toHaveLength(1)
    expect(w.text()).toContain('Use ast.literal_eval')
  })

  it('free-text search matches remediation text', async () => {
    const w = mountRules()
    await flushPromises()
    await w.get('[data-test=rule-search]').setValue('literal_eval')
