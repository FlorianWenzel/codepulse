...
total = len(d)
langs = sorted(bylang)
real_langs = [l for l in langs if l not in ("any", "docker", "github-actions")]
...
L.append(f"**{total} built-in rules** across **{len(real_langs)} languages** (plus secret detection, Dockerfile & GitHub Actions checks). Each rule carries a")
